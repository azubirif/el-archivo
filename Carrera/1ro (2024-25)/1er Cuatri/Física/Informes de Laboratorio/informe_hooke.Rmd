...
### Cálculo de errores
Antes de proceder con el cálculo de las constantes, es necesaria una meditación sobre los errores. Los errores (o precisión) a causa de los instrumentos de medición son los siguientes:
- Tiempo: $\Delta t = 0.01~(s)$
- Distancia: $\Delta y = 0.5~ (mm)$
- Masa: $\Delta m = 0.005~ (kg)$
Por tanto, vamos a pasar a calcular los errores de las diferentes cantidades medidas. Para empezar, la constante en función de la masa y el desplazamiento es:
$$
k = \frac{mg}{y}
$$
Y por tanto
$$
\Delta k = |\frac{\partial k}{\partial m}|\Delta m + |\frac{\partial k}{\partial y}|\Delta y = \frac{g}{y}\Delta m +\frac{mg}{y^2}\Delta y = \frac{g}{y}(\Delta m + \frac{m}{y}\Delta y)
$$
...
$$
k_1 = \frac{g}{m_1} = 0.0191 (\frac{N}{mm}) = 19.155 ~(\frac{N}{m})
$$
$$
k_2 = \frac{g}{m_2} = 0.00289 (\frac{N}{mm}) = 2.8908 ~(\frac{N}{m})
$$
$$
k_3 = \frac{g}{m_3} = 0.002525 (\frac{N}{mm}) = 2.5277~ (\frac{N}{m})
$$
...
```{r echo=FALSE}
plot(datos$`Período (s)...5`, sqrt(datos$`Masa (kg)...2`), data = datos)
plot(datos$`Período (s)...13`, sqrt(datos$`Masa (kg)...10`), data = datos)
plot(datos$`Período (s)...18`, sqrt(datos$`Masa (kg)...15`), data=datos)
```
...
- El movimiento provocado por desplazar el peso una cantidad $A$ de la posición de equilibrio genera un movimiento armónico simple dado por la ecuación $y(t)=A\sin (\omega t + \delta_0)$.
Debido a las condiciones del experimento, existen varios errores a consecuencia de los métodos de medición:
- Precisión de los artilugios utilizados, como una regla, cronómetro, báscula.
- Errores inevitables causados a causa de ser personas quienes comenzaban y detenían la cuenta del reloj, provocados por los reflejos de los medidores.
- Posibles fallos o desgastes en los instrumentos.
Pese a esto, se han podido sacar conclusiones exitosas sobre los experimentos.

# Conclusión
Finalmente, podemos concluir que la ley de Hooke  se ha podido verificar experimentalmente, y que permite predecir el movimiento de sistemas afectados por muelles individuales o acoplados.
